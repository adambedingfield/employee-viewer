...
function addEmployee() {
  ...
  db.query(sql, (error, response) => {
      ...
      const isManagerArray = [val.first, val.last];
      ...
      db.query(getManager, (error, response) => {
        ...
        inquirer.prompt(
        {
          ...
          // prompt to determine whether new employee is a manager or not
          inquirer.prompt(
            {
              type: 'list',
              name: 'confirmManager',
            ...
            // Insert employee data into manager SQL table
            if (val.confirmManager === 'Yes') {
              const sql = `INSERT INTO manager (first_name, last_name) VALUES (?, ?)`;
              db.query(sql, isManagerArray, (error) => {
                if (error) throw error;
                const mainPrompt = require('./prompt.js'); mainPrompt();
              })
            } else {
